<?php
/**
 * 用户课程相关的数据管理获取service
 * @文件名称: UserCourseService
 * @author: jawei
...
use appcomponents\modules\common\models\UserCourseModel;
...
class UserCourseService extends BaseService
{
    ...
    public function getCount($params = []) {
        $newsModel = new UserCourseModel();
        $count = $newsModel->getCount($params);
    ...
    public function addAll($dataAll) {
        if(empty($dataAll) || !isset($dataAll[0])) {
            return BaseService::returnErrData([], 56900, "用户课程批量数据入库异常");
        }
        $newsModel = new UserCourseModel();
        ...
        return BaseService::returnErrData([], 510000, "用户课程批量数据入库异常");
    }
}
