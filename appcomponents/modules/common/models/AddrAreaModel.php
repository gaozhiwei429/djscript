...
use Yii;
class AddrAreaModel extends BaseModel {
    //是否有效[1有效，2无效，0屏蔽
    const IS_STATUS = 1;
    const NO_STATUS = 2;
    const NOT_STATUS = 0;
    ...
    public function addInfo($addData) {
        try {
            $thisModel = new self();
            $thisModel->id = isset($addData['id']) ? trim($addData['id']) : null;//id
            $thisModel->city_id = isset($addData['city_id']) ? trim($addData['city_id']) : null;//文件服务器存储位置
            ...
            $thisModel->is_delete = isset($addData['is_delete']) ? trim($addData['is_delete']) : null;//is_delete
            $thisModel->enable = isset($addData['enable']) ? trim($addData['enable']) : null;//enable
            $thisModel->order_id = isset($addData['order_id']) ? trim($addData['order_id']) : null;//order_id
...
//            return $isSave;
            ...
            DmpLog::error('insert_addr_area_error', $log);
            return false;
        }
    }
    /**
    ...
    public static function updateInfo($id, $updateInfo) {
        try {
            ...
            DmpLog::error('update_addr_area_error', $log);
            return false;
        }
    }
    /**
...
//        return $query->createCommand()->getRawSql();
    ...
    public static function getListData($params = [], $orderBy = [], $offset = 0, $limit = 10, $fied=['*']) {
        try {
            ...
            DmpLog::warning('getListData_addr_area_error', $log);
            return [];
        }
    }
    /**
...
//                return $query->createCommand()->getRawSql();
            ...
            DmpLog::warning('getCount_addr_area_error', $log);
            return 0;
        }
    }
}
