...
use Yii;
class AddrCityModel extends BaseModel {
    //是否有效[1有效，2无效，0屏蔽
    const IS_STATUS = 1;
    const NO_STATUS = 2;
    const NOT_STATUS = 0;
    ...
    public function addInfo($addData) {
        try {
            ...
            $thisModel->province_id = isset($addData['province_id']) ? trim($addData['province_id']) : null;
            ...
            $thisModel->city_code = isset($addData['city_code']) ? trim($addData['city_code']) : null;//城市标识码
            $thisModel->save();
...
//            return $isSave;
            ...
            DmpLog::error('insert_addr_city_model_error', $log);
            return false;
        }
    }
    /**
    ...
    public static function updateInfo($id, $updateInfo) {
        try {
            ...
            DmpLog::error('update_addr_city_model_error', $log);
            return false;
        }
    }
    /**
...
//        return $query->createCommand()->getRawSql();
    ...
    public static function getListData($params = [], $orderBy = [], $offset = 0, $limit = 10, $fied=['*']) {
        try {
            ...
            DmpLog::warning('getListData_addr_city_error', $log);
            return [];
        }
    }
    /**
...
//                return $query->createCommand()->getRawSql();
            ...
            DmpLog::warning('getCount_addr_city_model_error', $log);
            return 0;
        }
    }
}
