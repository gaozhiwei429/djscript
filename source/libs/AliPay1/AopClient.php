...
//		$logger->conf["separator"] = "^_^";
        ...
        DmpLog::debug("logCommunicationError");
        DmpLog::debug($logData);
//		$logger->log($logData);
	}
...
DmpLog::debug($httpmethod);
		if ("GET" == $httpmethod) {

			//拼接GET请求串
			$requestUrl = $this->gatewayUrl."?".$preSignStr."&sign=".urldecode($totalParams["sign"]);

            DmpLog::debug($requestUrl);
			return $requestUrl;
		} else {
			//拼接表单字符串
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		foreach ($sysParams as $sysParamKey => $sysParamValue) {
			$requestUrl .= "$sysParamKey=" . $this->characet($sysParamValue, $this->postCharset) . "&";
		}
        foreach ($apiParams as $apiParamKey => $apiParamValue) {
//            $requestUrl .= "$apiParamKey=" . $this->characet($apiParamValue, $this->postCharset) . "&";
        }
